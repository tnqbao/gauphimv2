...
const Detail: React.FC<{ filmDetails: Film; error?: string }> = ({ filmDetails, error }) => {
    ...
    }, [dispatch, filmDetails, error]);

    if (error) {
        return <div>Không thể tải thông tin phim. Vui lòng thử lại sau.</div>;
    }

    const pageTitle = filmDetails.name;
    const pageDescription =
        filmDetails.content.length > 100
            ? `${filmDetails.content.slice(0, 100)}...`
            : filmDetails.content;
    const pageUrl = `https://gauphim.daudoo.com/detail/${filmDetails.slug}`;
    const pageImage = `https://img.ophim.live/uploads/movies/${filmDetails.poster_url}`;
...
export const getServerSideProps: GetServerSideProps = async ({ locale, query }) => {
    ...
    try {
        ...
        const episodes = Array.isArray(data.item.episodes[0]?.server_data)
            ? data.item.episodes[0].server_data.map((episode: { name: string; link_m3u8: string }) => ({
                name: episode.name,
